...
type TaoXManagedState struct {
	*TradingStateDB
	mu        sync.RWMutex
	exchanges map[common.Hash]*exchanges
}

// TaoXManagedState returns a new managed state with the statedb as it's backing layer
func ManageState(statedb *TradingStateDB) *TaoXManagedState {
	return &TaoXManagedState{
...
func (ms *TaoXManagedState) SetState(statedb *TradingStateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *TaoXManagedState) RemoveNonce(addr common.Hash, n uint64) {
...
func (ms *TaoXManagedState) NewNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *TaoXManagedState) GetNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	if ms.hasAccount(addr) {
		account := ms.getAccount(addr)
...
func (ms *TaoXManagedState) SetNonce(addr common.Hash, nonce uint64) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *TaoXManagedState) HasAccount(addr common.Hash) bool {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *TaoXManagedState) getAccount(addr common.Hash) *exchanges {
